...
// 
// this file can be implemented by macros.
// like #define PUSH(info,a) write_byte(info->memory, BASE_STACK + info->registers.SP--, a);
...
static void log(p_nes_cpu_info info, int size)
{
#ifdef _DEBUG
	memset(logs, 0x0, sizeof(logs));
	ubyte code = read_byte(info->memory, info->registers.PC);
	if (size == 0)
	{
		sprintf(logs, "%04X  %02X  %s",
			info->registers.PC, code, opcodes_name[code]);
	}
	else if (size == 1)
	{
		ubyte operand = read_byte(info->memory, info->registers.PC + 1);
		sprintf(logs, "%04X  %02X %02X  %s $%02X",
			info->registers.PC, code, operand, opcodes_name[code], operand);
	}
	else
	{
		uword operand = read_word(info->memory, info->registers.PC + 1);
		sprintf(logs, "%04X  %02X %02X %02X  %s $%04X",
			info->registers.PC, code, ((ubyte)(operand & 0xFF)),
			((ubyte)(operand >> 8)), opcodes_name[code], operand);
	}
	printf(
		"%s                    A:%02X X:%02X Y:%02X P:%02X SP:%02X\n",
		logs, info->registers.A,
		info->registers.X,
		info->registers.Y,
		info->registers.P,
		info->registers.SP
...
// stack
static inline void push_byte(p_nes_cpu_info info, ubyte data)
{
	uword address = BASE_STACK + info->registers.SP--;
	write_byte(info->memory, address, data);
}

static inline ubyte pop_byte(p_nes_cpu_info info)
{
	uword address = BASE_STACK + ++info->registers.SP;
	return read_byte(info->memory, address);
}

static inline void push_word(p_nes_cpu_info info, uword data)
{
	push_byte(info, data >> 8);
	push_byte(info, data & 0xFF);
}

static inline uword pop_word(p_nes_cpu_info info)
{
...
static inline ubyte A_ZPX(p_nes_cpu_info info)
{
	...
	log(info->hardware, 2);
#endif
	uword address = read_byte(info->memory, info->registers.PC + 1) + info->registers.X;
	info->registers.PC += 2;
...
static inline ubyte A_ZPY(p_nes_cpu_info info)
{
	...
	log(info->hardware, 2);
#endif
	uword address = read_byte(info->memory, info->registers.PC + 1) + info->registers.Y;
	info->registers.PC += 2;
	return (ubyte)(address & 0xff);
}
...
static inline uword A_ABX(p_nes_cpu_info info)
{
	...
	log(info->hardware, 2);
#endif
	uword address = read_word(info->memory, info->registers.PC + 1) + info->registers.X;
	info->registers.PC += 3;
	return address;
}
static inline uword A_ABY(p_nes_cpu_info info)
{
	...
	log(info->hardware, 2);
#endif
	uword address = read_word(info->memory, info->registers.PC + 1) + info->registers.Y;
	info->registers.PC += 3;
	return address;
}
...
#endif
	uword operand = read_word(info->memory, info->registers.PC + 1);
	uword address = read_word_jump_ind(info->memory, operand);
	info->registers.PC += 3;
	return address;
}
...
static inline sbyte A_REL(p_nes_cpu_info info)
{
	...
	log(info->hardware, 1);
#endif
	sbyte operand = (sbyte)read_byte(info->memory, info->registers.PC + 1);
	info->registers.PC += 2;
	return operand;
}
static inline uword A_IZX(p_nes_cpu_info info)
{
	...
	info->registers.PC += 2;
	return zp_read_word(info->memory, address);
}
...
#endif
	ubyte operand = read_byte(info->memory, info->registers.PC + 1);
	uword address = zp_read_word(info->memory, operand) + info->registers.Y;
	info->registers.PC += 2;
	return address;
}
...
{
	return read_byte(info->memory, A_IZX(info));
}
static inline ubyte D_IZY(p_nes_cpu_info info)
{
	return read_byte(info->memory, A_IZY(info));
}
static inline ubyte D_ZP(p_nes_cpu_info info)
{
	return zp_read_byte(info->memory, A_ZP(info));
}
static inline ubyte D_ZPX(p_nes_cpu_info info)
{
	return zp_read_byte(info->memory, A_ZPX(info));
}
static inline ubyte D_ZPY(p_nes_cpu_info info)
{
	return zp_read_byte(info->memory, A_ZPY(info));
}
static inline ubyte D_ABS(p_nes_cpu_info info)
{
	return read_byte(info->memory, A_ABS(info));
}
static inline ubyte D_ABX(p_nes_cpu_info info)
{
	return read_byte(info->memory, A_ABX(info));
}
static inline ubyte D_ABY(p_nes_cpu_info info)
{
	return read_byte(info->memory, A_ABY(info));
}
...
static inline void dec_iml_a(p_nes_cpu_info info, func_uword func)
{
	...
	ubyte data = read_byte(info->memory, address) - 1;
	write_byte(info->memory, address, data);
	set_n_z_flag(info, data);
}
static inline void dec_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	ubyte address = func(info);
	ubyte data = zp_read_byte(info->memory, address) - 1;
	zp_write_byte(info->memory, address, data);
	set_n_z_flag(info, data);
}
...
{
	uword address = func(info);
	ubyte data = read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	write_byte(info->memory, address, g_asl_table[data].value);
	set_flag(info, g_asl_table[data].flag);
}
static inline void asl_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	ubyte address = func(info);
	ubyte data = zp_read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	zp_write_byte(info->memory, address, g_asl_table[data].value);
	set_flag(info, g_asl_table[data].flag);
}
static inline void rol_iml_a(p_nes_cpu_info info, func_uword func)
{
	...
	ubyte data = read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	write_byte(info->memory, address, g_rol_table[tmp][data].value);
...
static inline void rol_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	...
	ubyte data = zp_read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	zp_write_byte(info->memory, address, g_rol_table[tmp][data].value);
	set_flag(info, g_rol_table[tmp][data].flag);
}
static inline void lsr_iml_a(p_nes_cpu_info info, func_uword func)
{
	...
	ubyte data = read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	write_byte(info->memory, address, g_lsr_table[data].value);
...
static inline void lsr_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	...
	ubyte data = zp_read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	zp_write_byte(info->memory, address, g_lsr_table[data].value);
	set_flag(info, g_lsr_table[data].flag);
}
static inline void ror_iml_a(p_nes_cpu_info info, func_uword func)
{
	...
	ubyte data = read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	write_byte(info->memory, address, g_ror_table[tmp][data].value);
...
static inline void ror_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	...
	ubyte data = zp_read_byte(info->memory, address);
	reset_flag(info, FLAG_N | FLAG_Z | FLAG_C);
	zp_write_byte(info->memory, address, g_ror_table[tmp][data].value);
	set_flag(info, g_ror_table[tmp][data].flag);
}
// move commands
...
{
	uword address = func(info);
	write_byte(info->memory, address, info->registers.A);
}
static inline void sta_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	ubyte address = func(info);
	zp_write_byte(info->memory, address, info->registers.A);
}
...
{
	uword address = func(info);
	write_byte(info->memory, address, info->registers.X);
}
static inline void stx_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	ubyte address = func(info);
	zp_write_byte(info->memory, address, info->registers.X);
}
...
{
	uword address = func(info);
	write_byte(info->memory, address, info->registers.Y);
}
static inline void sty_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	ubyte address = func(info);
	zp_write_byte(info->memory, address, info->registers.Y);
}
// branch
...
{
	uword address = func(info);
	write_byte(info->memory, address, info->registers.A & info->registers.X);
}
static inline void sax_iml_a_zp(p_nes_cpu_info info, func_ubyte func)
{
	ubyte address = func(info);
	zp_write_byte(info->memory, address, info->registers.A & info->registers.X);
}
...
static inline void BRK(p_nes_cpu_info info)
{
	...
	push_byte(info, info->registers.P);
	set_flag(info, FLAG_I);
	reset_flag(info, FLAG_D);
	info->registers.PC = read_word(info->memory, IRQ_VECTOR);
}
...
static inline void PHP(p_nes_cpu_info info)
{
	...
	push_byte(info, info->registers.P | FLAG_B | FLAG_R);
}
...
static inline void JSR_ABS(p_nes_cpu_info info)
{
	...
	push_word(info, info->registers.PC - 1);
...
static inline void RTS(p_nes_cpu_info info)
{
	// 60
	IMP_ACC(info);
	info->registers.PC = pop_word(info) + 1;
}
...
static inline void LAS_ABY(p_nes_cpu_info info)
{
	...
	ubyte data = read_byte(info->memory, address) & info->registers.SP;
