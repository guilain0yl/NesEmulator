...
static inline uword A_IZX(p_nes_cpu_info info)
{
	...
	info->registers.PC += 2;
	return zp_read_word(((p_nes_hardware_info)info->hardware)->mem_info, address);
}
...
static inline void PHP(p_nes_cpu_info info)
{
	...
	push_byte(((p_nes_hardware_info)info->hardware)->mem_info, info->registers.P | FLAG_B | FLAG_R);
}
...
static inline void JSR_ABS(p_nes_cpu_info info)
{
	...
	push_word(((p_nes_hardware_info)info->hardware)->mem_info, info->registers.PC - 1);
...
static inline void RTS(p_nes_cpu_info info)
{
	// 60
	IMP_ACC(info);
	info->registers.PC = pop_word(((p_nes_hardware_info)info->hardware)->mem_info) + 1;
}
